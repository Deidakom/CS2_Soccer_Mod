...
test('Ball size: five exact sizes in the Ball menu, applied live with the SetScale input', () => {
  ...
  for (const [name, value] of [['CS2 Legacy', '1f'], ['10% smaller', '0.9f'], ['Default (13% smaller)', 'DefaultBallSize'],
    ['15% smaller', '0.85f'], ['20% smaller', '0.8f']])
    assert.ok(size.includes(`("${name}", ${value})`), name);
  assert.ok(read('SoccerModMvpPlugin.cs').includes('private const float DefaultBallSize = 0.87f;'), '13% smaller is the default (owner, 2026-09-25)');
